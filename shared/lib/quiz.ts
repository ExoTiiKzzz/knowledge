import { COUNTRIES, type Continent, type Country } from '../data/countries.ts'
import { MAP_SHAPES } from '../data/map.ts'
import { checkAnswer, closestMatch, withinTolerance, type Verdict } from './answer.ts'
 ...
 *
 * `random` par défaut sur l'aléatoire global : le solo n'a besoin de reproduire
 * aucun tirage. Le multijoueur passe un générateur ensemencé, afin qu'une même
 * graine rende toujours la même série.
 */
export function buildQuiz(
  ...
  const pool = shuffle(countriesIn(scope, mode), random)
  const picked = count === null ? pool : pool.slice(0, count)
...
export function retryQuiz(answered: Answered[], random: Random = Math.random): Question[] {
  return shuffle(
    answered
